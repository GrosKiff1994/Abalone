...
public class SuperController {
	...
	public int compteCouleur(Direction delta, int iDep, int jDep,
			Couleur couleur) {
		int nbCoul = 0;
		...
		while (plateau.getCase(parcours) != null
				&& plateau.getCase(parcours).estOccupee()
	...
	public void verifierBoules() {
		...
		for (int i = 0; i < Plateau.HEIGHT; i++) {
			for (int j = 0; j < Plateau.WIDTH; j++) {
				if (plateau.getCase(i, j).getBord()
						&& plateau.getCase(i, j).estOccupee()) {
					for (Joueur joueur : modele.getTabJoueurs()) {
						if (plateau.getCase(i, j).getBoule().getCouleur() == joueur
								.getCouleur()) {
	...
	public void sourisRelachee(MouseEvent e) {
		...
		System.out.println("clic : ligne " + bouton.getCoordI() + ", colonne "
				+ bouton.getCoordJ());
		...
		case NORMAL:
			...
			case CLICGAUCHE:
				...
				for (Direction dir : lesDir) {
					Coord dest = new Coord(bouton.getCoordJ() + dir.getX(),
							bouton.getCoordI() + dir.getY());
			...
			case CLICDROIT:
				...
				// afficher cercle voisins
				for (Direction dir : lesDir) {
					Case caseDest = plateau.getCase(
							bouton.getCoordI() + dir.getY(), bouton.getCoordJ()
									+ dir.getX());
					...
					if (tmp != null
							&& !caseDest.getBord()
							&& caseDest.estOccupee()
							&& caseDest.getBoule().getCouleur() == plateau
									.getCase(depart).getBoule().getCouleur()) {
						tmp.setCouleurActuelle(BoutonRond.couleurLigne);
						tmp.setVisible(true);
					}
				}
		...
		case SELECTIONLIGNE:
			...
			case CLICGAUCHE:
				...
				Couleur couleurDepart = plateau.getCase(depart).getBoule()
						.getCouleur();
				int nbCouleurActuelle = compteCouleur(dir, depart,
						couleurDepart);
				int nbCouleurOpposee = 0;

				Case caseDeFinCouleurActuelle = plateau.getCase(depart.getY()
						+ nbCouleurActuelle * delta.getY(), depart.getX()
						+ nbCouleurActuelle * delta.getX());

				/* seconde ligne de boules */
				if (caseDeFinCouleurActuelle.estOccupee()) {
					Couleur couleurArr = caseDeFinCouleurActuelle.getBoule()
							.getCouleur();
					nbCouleurOpposee = compteCouleur(dir, depart.getY()
							+ nbCouleurActuelle * delta.getY(), depart.getX()
							+ nbCouleurActuelle * delta.getX(), couleurArr);
				}
				...
				Case caseFinale = plateau.getCase(depart.getY() + coeffDelta
						* delta.getY(),
						depart.getX() + coeffDelta * delta.getX());
				if (caseFinale != null && caseFinale.estOccupee()) {
					deplacementPossible = false;
				}

				System.out.println("nbCouleurAcuelle = " + nbCouleurActuelle
						+ ", nbCouleurOpposee = " + nbCouleurOpposee);

				// deplacement reel
				if (nbCouleurActuelle <= Plateau.MAXDEPLACEMENT
						&& nbCouleurOpposee < nbCouleurActuelle
						&& deplacementPossible) {
		...
		case SELECTIONLATERAL:
			switch (e.getButton()) {
			case CLICDROIT:
				Coord arrivee = new Coord(bouton.getCoordJ(),
						bouton.getCoordI());
				Coord delta = new Coord(arrivee.getX() - depart.getX(),
						arrivee.getY() - depart.getY());

				panneau.cacherBoutons();

				/* caseDecalDepart - caseDepart - caseArrivee - caseDecalArrivee */
				Case caseDecalArrivee = plateau.getCase(
						arrivee.getY() + delta.getY(),
						arrivee.getX() + delta.getX());
				Case caseDecalDepart = plateau.getCase(
						depart.getY() - delta.getY(),
						depart.getX() - delta.getX());
				...
				if (caseDecalArrivee.estOccupee()
						&& caseDecalArrivee.getBoule().getCouleur() == caseArrivee
								.getBoule().getCouleur()) {
					caseDecalArrivee.getBouton().setCouleurActuelle(
							BoutonRond.couleurLigne);
					caseDecalArrivee.getBouton().setVisible(true);
				}

				if (caseDecalDepart.estOccupee()
						&& caseDecalDepart.getBoule().getCouleur() == caseDepart
								.getBoule().getCouleur()) {
					caseDecalArrivee.getBouton().setCouleurActuelle(
							BoutonRond.couleurLigne);
				...
				for (Direction dir : lesDir) {
					...
					caseDest = plateau.getCase(caseDepart.getBouton()
							.getCoordI() + dir.getY(), caseDepart.getBouton()
							.getCoordJ() + dir.getX());

					tmp = caseDest.getBouton();
					if (tmp != null && !caseDest.getBord()
							&& !caseDest.estOccupee()) {
						tmp.setCouleurActuelle(BoutonRond.couleurLateralSelec);
						tmp.setVisible(true);
					}

					caseDest = plateau.getCase(caseArrivee.getBouton()
							.getCoordI() + dir.getY(), caseArrivee.getBouton()
							.getCoordJ() + dir.getX());
	...
	private void deplacerLigneBoules(int nbBoules, Coord delta) {
		...
		Coord coordDepla = new Coord(depart.getX() + (nbBoules - 1)
				* delta.getX(), depart.getY() + (nbBoules - 1) * delta.getY());
		while (nbBoules > 0) {
			nbBoules--;
			try {
				plateau.deplacerBouleDirection(Direction.toDirection(delta),
						coordDepla);
	...
	public void sourisEntree(MouseEvent e) {
		...
		if (bouton.getCouleurActuelle() == BoutonRond.couleurLateralDeplac
				|| etat == Etat.SELECTIONLATERAL2) {
			bouton.setCouleurActuelle(BoutonRond.couleurMouseOver);
			for (Direction dir : lesDir) {
				Case caseDest = plateau.getCase(
						bouton.getCoordI() + dir.getY(), bouton.getCoordJ()
								+ dir.getX());
				if (caseDest.getBouton().isVisible()
						|| caseDest.getBouton().getCouleurActuelle() == BoutonRond.couleurLateralDeplac) {
					caseDest.getBouton().setCouleurActuelle(
							BoutonRond.couleurMouseOver);
					System.out.println("test");
				} else {
					caseDest = plateau.getCase(bouton.getCoordI() - dir.getY(),
							bouton.getCoordJ() - dir.getX());
					if (caseDest.getBouton().isVisible()
							|| caseDest.getBouton().getCouleurActuelle() == BoutonRond.couleurLateralDeplac) {
						caseDest.getBouton().setCouleurActuelle(
								BoutonRond.couleurMouseOver);
			...
			// ajouter le bouton dans la direction :
			// si bouton "coord bouton + dir" existe allumer coords bouton + dir
			// si bouton "coord bouton + dir" n'existe pas... coord bouton - dir
		}
		if (bouton.getCouleurActuelle() == BoutonRond.couleurLateralSelec
				|| etat == Etat.SELECTIONLATERAL3) {
			bouton.setCouleurActuelle(BoutonRond.couleurMouseOver);
			// ajouter les boutons dans la direction...
			// QUE FAIRE ? boutons 1,2,3
		}
	...
	public void sourisSortie(MouseEvent e) {
		...
		if (bouton.getCouleurActuelle() == BoutonRond.couleurMouseOver) {
			...
			} else if (etat == Etat.SELECTIONLIGNE
					|| etat == Etat.SELECTIONLATERAL
					|| etat == Etat.SELECTIONLATERAL2
					|| etat == Etat.SELECTIONLATERAL3) {
